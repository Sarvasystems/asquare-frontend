...
import './AddCommercialListing.css'; 

const AddCommercialListing = () => {
  ...
  const handleAddListing = async () => {
    try {
      const response = await axios.post('http://localhost:5000/api/CommercialListings/', listingData);
  ...
  const handleUploadImages = async () => {
    try {
      ...
      const response = await axios.post(`http://localhost:5000/api/CommercialListings/upload/${uploadedId}`, formData, {
        headers: {
...
export default AddCommercialListing;
