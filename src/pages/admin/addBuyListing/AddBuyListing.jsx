...
const AddBuyListing = () => {
  ...
  const handleAddListing = async () => {
    try {
      const response = await axios.post('http://localhost:5000/api/BuyListings/', listingData);
  ...
  const handleUploadImages = async () => {
    try {
      ...
      const response = await axios.post(`http://localhost:5000/api/BuyListings/upload/${uploadedId}`, formData, {
        headers: {
